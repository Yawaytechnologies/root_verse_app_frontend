// src/components/crate_packer/modals/common.tsx
...
export function Label({ children }: { children: React.ReactNode }) {
  return <Text className="mt-3 text-[13px] font-extrabold text-white/70">{children}</Text>;
}
        ...
        <View className="flex-1 bg-black/60 p-4 justify-center">
          <View className="max-h-[80%] rounded-3xl border border-white/10 bg-[#0b1630] overflow-hidden">
            <View className="flex-row items-center justify-between px-4 py-4 border-b border-white/10">
              <Text className="text-white font-extrabold text-[16px]">{placeholder || "Select"}</Text>
              <Pressable onPress={() => setOpen(false)} className="px-3 py-2">
                <Text className="text-white/85 font-extrabold text-[18px]">✕</Text>
              </Pressable>
            </View>
                ...
                return (
                  <Pressable
                    ...
                    className={[
                      ...
                      active ? "border-blue-400/50 bg-blue-500/20" : "border-white/10 bg-white/5",
                    ].join(" ")}
                  >
                    <Text className="text-white font-extrabold">{String(it)}</Text>
                  </Pressable>
                );
              })}
            </ScrollView>
 ...
 *
 * ✅ Also fixes TS typing mismatch:
 * some expo-file-system typings in your project may not expose cacheDirectory/documentDirectory,
...
export async function ensureFileUri(inputUri: string): Promise<string> {
  ...
  // ✅ crate packer preview dir
  const dir = `${baseDir}crate_preview/`;
  try {
  ...
  const dest = `${dir}crate_${Date.now()}_${Math.random().toString(16).slice(2)}.${ext}`;
    ...
    <>
      <View className="mt-3 gap-3">
        {uris.map((uri) => {
          ...
          return (
            <View key={uri} className="overflow-hidden rounded-2xl border border-white/10 bg-white/5">
              {/* ✅ thumbnail */}
              <Pressable
                onPress={async () => {
                  setPreviewErr(null);
      ...
      {/* ✅ Preview modal */}
      <Modal visible={!!previewUri} transparent={false} animationType="fade" onRequestClose={() => setPreviewUri(null)}>
          ...
          <View className="flex-1 p-4">
            <View className="flex-1 rounded-2xl overflow-hidden bg-black/20">
              ...
              {previewLoading && (
                ...
                >
                  <Text style={{ color: "rgba(255,255,255,0.85)", fontWeight: "900" }}>Loading…</Text>
                </View>
              )}
              ...
              {!!previewErr && (
                ...
                >
                  ...
                  >
                    If Android returns a content:// URI, preview can fail. This screen tries to copy it to file://.
                    If it still fails, capture again or pick again.
                  </Text>
                </View>
              )}
            </View>
